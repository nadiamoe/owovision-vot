<?php
	...
	if ($getNewUser)
	{
		...
		if ($row == false)
		{
			...
			$statement->bindValue(":handleUser", $newUser);
			$statement->bindValue(":hasVoted", $hasVoted);
			$statement->execute();
...
<body>
<?php
			$statement = $db->prepare("SELECT value FROM SETTINGS WHERE key='voteOpen'");
			$result = $statement->execute();
			$row = $result->fetchArray();

			if($row['value'] == 0)
			{
				die("<p>¡La votación ya se ha cerrado!</p>");
			}
			...
			?>
	<p class="center" style="margin-bottom:0;">¡Hola, <?php echo($row["handleUser"]); ?>! Por favor, elige tus votos arrastrando los nombres de los países:</p>
